...
class ConfigSVM():
    ...
    def __str__(self):
        print_order = ["Manual_Feats", "Return", "Parameters", "Description", "SigFeatures"]
        ret = ""
        for feat_name in print_order:
            if feat_name not in self.feat_dict.keys():
                continue
            #     raise ValueError(feat_name+" not in feature list. Must be in: "+str(list(self.feat_dict.keys())))
            if self.feat_dict[feat_name] is not None:
                ret += "Feature set: "+feat_name+"\n\t"+str(self.feat_dict[feat_name])+"\n"

        if ret == "":
            ValueError("Configuration is empty. Logic error ocurred")
    ...
    def build_svm_feat_dict(self, feat_search_list, built_so_far:dict, store_completed):
        ...
        if len(feat_search_list) == 1:
            ...
            if feat_name == "Manual_Feats":
                # Don't save a feat dict with only {'Manual_Feats': False}
                if len(built_so_far.keys()) == 0:
                    if True in feat_space:
                        store_completed.append({feat_name:True})
                return
                ...
                for feat_config in feat_space.generator():
                    ...
                    # Make explicit in svm configuration
                    if "Manual_Feats" not in cpy.keys():
                        cpy["Manual_Feats"] = False
